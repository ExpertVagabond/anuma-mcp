...
Anuma markets long-term memory and it exists, but not as an API you call.
`/api/v1/responses` is stateless: tell it your codename in one request and it
does not know in the next. `conversation_id` is documented "pass-through only,
not forwarded to the LLM provider", so it is observability. There is no memory
or vault path anywhere in the API's 181.

That is by design. The engine ships in `anuma-ai/sdk` as client-side code
(`src/lib/memory/`: extraction, decay, RRF, MMR, salience, graph traversal,
consolidation) over a local encrypted vault, and `assembleMemoryContext()`
builds the prompt **before** the request is sent. The server never holds your
history, which is the point of a private AI layer.

So a client that wants memory implements it. This one does:

```
anuma_remember   store a fact, typed
anuma_recall     search the vault
anuma_forget     delete one fact, or prune everything expired
anuma_respond    with memory:true, recalls and prepends before sending
```

```jsonc
{ "text": "My project codename is Kestrel", "type": "project" }
{ "model": "...", "memory": true, "prompt": "What is my project codename?" }
// -> "Your project codename is Kestrel."
//    memoryUsed: [{ "id": "m_...", "text": "My project codename is Kestrel" }]
```

Facts are typed because type decides how long they stay true, which is a
different question from how much they matter: `identity` never expires,
`preference` lasts a year, `project` 90 days, `event` 14. **Decay runs from last
use, not from creation**, so recalling a fact keeps it alive and neglect is what
kills it. Recall scores term overlap first and uses recency only to break ties,
because a memory that is not about the question is not worth surfacing however
fresh it is. Near-duplicates are dropped rather than spending context saying the
same thing twice.

The vault is a plain JSON file at `~/.anuma-mcp/memory.json`
(`ANUMA_MEMORY_PATH` overrides) and is **not encrypted**. The SDK encrypts
because it syncs; this never leaves the machine, so encrypting it would trade
real inspectability for the appearance of security. Read it, diff it, delete it.

Anuma sees the assembled text and never the vault, which is the same division
the SDK draws. Multi-turn remains available for when you want exact history
rather than recalled facts: resend the conversation via `messages`.
